...
### 新增功能 - 全业务覆盖模板体系（解决输出内容单一问题）

针对用户反馈"改写出来的文章只有专利和项目类型"的问题，从配置层面全面优化，确保商标、版权、认证、海外布局等业务类型都能得到正确输出。

#### 1. 4个现有模板company_background全业务覆盖
所有模板的company_background统一更新为全业务描述，覆盖商标代理、专利申请、版权登记、体系认证、科创项目申报、海外知识产权布局。

#### 2. style_rules业务融入要求改为按客户群体匹配
将硬性的"自然融入专利、软著、研发费用归集、科创申报业务"改为按客户群体匹配：电商群体融入商标/版权，制造群体融入专利/高新，跨境群体融入海外商标/马德里，科技群体融入软著/专利/科创申报，不要强行套用不相关的业务。

#### 3. SEO核心关键词扩展为按客户群体区分
将固定的6个关键词扩展为按5个客户群体分别定义核心关键词，确保不同类型的文章都能正确使用对应的SEO关键词。

#### 4. 增加商标/版权/认证/海外布局的模板匹配关键词
4个现有模板各新增24个priority_keywords和32-35个match_keywords，覆盖商标类、版权类、认证类、海外类四大业务领域。

#### 5. 新增2个专用模板
- trademark_copyright（商标版权类）：适配商标注册、版权登记、电商维权、品牌保护类文章，match_priority=5
- overseas_ip（海外知识产权类）：适配海外商标、PCT专利、跨境维权、出海布局类文章，match_priority=6

#### 6. 模板总数从4个增加到6个
| 模板ID | 名称 | 优先级 | 适配场景 |
|---|---|---|---|
| policy_announcement | 政策公示类 | 1 | 入库名单、公示、新规、数据播报 |
| knowledge_share | 干货科普类 | 2 | 软著、专利、高企、财税等干货 |
| case_review | 客户案例复盘类 | 3 | 客户服务案例、申报复盘 |
| hot_topic | 热点事件类 | 4 | 科研发现、热点新闻+知识产权类比 |
| trademark_copyright | 商标版权类 | 5 | 商标注册、版权登记、电商维权 |
| overseas_ip | 海外知识产权类 | 6 | 海外商标、PCT专利、跨境维权 |
...
针对大模型难以完全避免章节编号输出的问题，新增后处理机制自动去除章节编号；同时优化重试逻辑，保留校验通过项最多的最优版本。

#### 1. 新增章节编号后处理函数
- 新增 `remove_section_numbers(content)` 函数，在输出前自动处理
- 自动检测并去除独立成行的章节编号（一、二、三、1. 2.、（一）（二）、第一第二等）
- 去除编号后自动将标题文字加粗（**标题**），保持结构清晰
- 跳过frontmatter、空行、已加粗行、markdown标题行，避免误处理
- 只处理独立成行的标题，不处理正文中的列表项

#### 2. 重试循环保留最优版本
- 重试循环中记录每次尝试的校验通过项数量（0-8项）
- 循环结束后自动选择通过项最多的最优版本输出
- 如果某次尝试全部通过（8/8），直接break使用该版本
- 日志中明确显示"使用第X次尝试的最优版本（通过Y/8项校验）"

#### 3. 输出格式要求加强
- 4个模板的task_template各增加5条格式强制要求
- 禁止输出「# 标题」「标题」「正文」「内容」等占位标记
- 禁止输出「# 正文」「---正文开始---」等格式分隔标记
- 标题行不需要加#号，第一行直接是文章标题（带【】前缀）
- 禁止输出「根据要求」「以下是改写后的文章」等AI说明性文字

#### 4. customer_segments.json增加专属痛点关键词
- 5个客户群体各增加专属痛点关键词，共111个
- 电商/个体户：21个（店铺被封、链接下架、商标被抢注、盗图等）
- 中小制造工厂：22个（产品被仿、高新没认下来、专利被驳回、拿不到补贴等）
- 跨境外贸：23个（海外商标被抢、海关扣货、亚马逊封号、侵权投诉等）
- 科技研发企业：26个（软著漏登记、发明专利周期长、申报不合格、研发费用混乱等）
- 通用中小企业：19个（知识产权成本高、申报流程复杂、怕踩坑被骗等）

#### 5. API调用间隔优化
- config.json新增 `api_call_interval_seconds = 5` 配置项
- 重试之间自动等待5秒，减少429限流概率
- 文件处理之间自动等待5秒，批量处理时更稳定
- 修复了file_index未定义的bug（文件循环未使用enumerate）

### Bug修复
- 修复 `remove_section_numbers` 函数中 `re` 未导入的问题
- 修复文件循环中 `file_index` 未定义的问题（添加enumerate）
- 修复标题检测函数的占位符识别bug（`# 【科技项目申报】标题`不再被误判为有效标题）
...
针对公众号文章"0流量"问题，建立完整的公众号运营SOP（标准作业程序），从内容创作到发布分发全流程标准化。

#### 1. 新增SOP文档
- 新增 `公众号运营SOP.md`，包含9大模块：标题规范、开头规范、内容规范、转发钩子规范、SEO规范、发布规范、分发规范、内容矩阵规范、自检清单

#### 2. 4个模板style_rules增加SOP强制要求
所有模板（政策公示类、干货科普类、客户案例复盘类、热点事件类）的style_rules新增【公众号传播SOP·强制要求】模块，包含5条硬性规则：
- **标题规范**：必须采用「痛点+悬念」结构，标题前半句必须出现具体痛点词，禁止纯行业热点名词开头
- **开头规范**：开头第一句必须是具体案例或痛点，禁止从新闻事件讲起，前100字必须出现痛点关键词
- **内容规范**：专业术语必须配大白话解释、必须有老板视角内容、所有数据必须标注来源或口径
- **转发钩子规范**：文末必须包含转发价值说明，明确告诉读者适合转发给谁以及转发的实际价值
- **SEO规范**：标题和开头必须包含核心关键词，全文自然重复3-5次

#### 3. 新增3项后置校验
- **标题痛点词检测**（`title_pain_keywords` + `min_title_pain_hits=1`）：检测标题是否包含痛点词（评不上、被刷下来、拿不到补贴等）
- **开头案例词检测**（`opening_case_keywords` + `min_opening_case_hits=2`）：检测文章开头前300字是否包含案例要素（具体城市、某企业、评审专家等）
- **转发钩子词检测**（`forward_hook_keywords` + `min_forward_hook_hits=2`）：检测文末300字是否包含转发钩子（转发给、研发负责人、项目申报等）

校验不通过自动重试，并在Prompt中追加对应强化要求。
...
### 配置变更
- `config.content` 新增6个配置项：`title_pain_keywords`（17个）、`opening_case_keywords`（18个）、`forward_hook_keywords`（13个）、`min_title_pain_hits`（默认1）、`min_opening_case_hits`（默认2）、`min_forward_hook_hits`（默认2）
- 4个模板的style_rules各增加6条SOP规则
- 版本号 1.4.2 → 1.5.0

### 不兼容变更
- 无（所有新增配置均有默认值，向下兼容；校验阈值可通过config调整）
